...
export function PointExchangeView({ setView }: { setView: (v: any) => void }) {
    ...
    return (
        <div className={`p-6 md:p-10 rounded-[32px] border shadow-sm ${isDark ? 'bg-gray-900 border-gray-800 text-white' : 'bg-white border-blue-100 text-gray-900'}`}>
            <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 bg-blue-50 text-[#1e3a5f] rounded-2xl flex items-center justify-center shrink-0">
                    <Gift size={24} />
                </div>
                <div>
            ...
            <div className={`p-6 rounded-3xl mb-8 border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4">
                    <div>
                        ...
                        <div className="text-3xl font-black text-[#1e3a5f] mt-1">{userPoints.toLocaleString()} <span className="text-lg">P</span></div>
                    </div>
                    {!canExchange && (
                ...
                <div>
                    <div className="flex justify-between items-center mb-1.5">
                        ...
                        <span className="text-[11px] font-black text-[#1e3a5f]">{Math.min(100, Math.round((userPoints / 2000) * 100))}% · {Math.min(userPoints, 2000).toLocaleString()} / 2,000P</span>
                    </div>
                    <div className={`w-full h-3 rounded-full overflow-hidden ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}>
                        <div
                            ...
                            style={{
                                ...
                                background: canExchange ? 'linear-gradient(90deg, #10b981, #34d399)' : 'linear-gradient(90deg, #1e3a5f, #fb7185)',
                            }}
                        />
                    </div>
                </div>
            ...
            {!submitted ? (
                <>
                                ...
                                className={`relative flex flex-col items-center justify-center p-6 border-2 rounded-3xl text-center transition-all ${
                                    !canExchange
                                        ...
                                        : selectedItem === item.id
                                            ? 'border-[#1e3a5f] bg-blue-50 shadow-lg shadow-sm'
                                            : isDark ? 'border-gray-700 hover:border-gray-500' : 'border-gray-200 hover:border-blue-300'
                                }`}
                            >
                                ...
                                <p className={`text-xs font-bold ${selectedItem === item.id ? 'text-[#1e3a5f]' : 'text-gray-400'}`}>{item.points.toLocaleString()}P 차감</p>
                    ...
                    <div className={`p-4 rounded-2xl mb-8 space-y-1 text-xs font-bold ${isDark ? 'bg-gray-800 text-gray-400' : 'bg-blue-50/50 text-gray-500'}`}>
                        <p className="text-[#1e3a5f] mb-2 font-black text-[13px]">💡 포인트를 모으는 방법! 가입만 해도 600P!</p>
                    ...
                    <button
                        ...
                        className={`w-full py-4 text-base font-black rounded-2xl transition-all ${
                            canExchange && selectedItem
                                ? 'bg-[#1e3a5f] text-white hover:bg-[#162d4a] shadow-xl shadow-sm active:scale-95'
